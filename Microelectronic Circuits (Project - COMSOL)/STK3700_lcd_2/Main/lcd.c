...
uint16_t adcSingle(void)
{
  ...
  ADC_InitSingle(ADC0,&singleInit); //mod jednostruka konverzija
  ...
  sampleValue=ADC_DataSingleGet(ADC0); //funkcija koja obradjuje i dohvata vrednost
...
uint16_t adcSingle1(void)
{
  ...
  ADC_InitSingle(ADC0,&singleInit1); //mod jednostruka konverzija
...
int main(void)
{
  ...
  GPIO_DriveModeSet(LIGHTSENSE_EXCITE_PORT, gpioDriveModeStandard); //Set DRIVEMODE as default (6 mA)
  GPIO_DriveModeSet(LIGHTSENSE_SENSOR_PORT, gpioDriveModeStandard); //Set DRIVEMODE as default (6 mA)
  GPIO_PinModeSet(LIGHTSENSE_EXCITE_PORT, LIGHTSENSE_EXCITE_PIN, gpioModePushPull,1); // Configure LIGHTSENSE_EXCITE_PIN as digital output (push-pull)
  GPIO_PinModeSet(LIGHTSENSE_SENSOR_PORT, LIGHTSENSE_SENSOR_PIN, gpioModeDisabled,0);
  GPIO_PinModeSet(gpioPortD,0, gpioModeInput,1); // Configure PD0 pin as analog input
  ...
  char messageMS[] = "MS:";
  char messageUmin[] = "Umin:";
  char messageUmax[] = "Umax:";
  ...
  while(1){
      ...
      long temp;
      volatile Umax,Umin1, Uoff, Uoff2;
      volatile U10, Utau;
      double voltage_predict;
      double temp_predict;
      double y0;
      double k;
      int time_constant=0.2;
      int x=0;
      double y=0;

      //Rezultat konverzije iz foto senzora
      while(adc_count2++<1000)
        adc_result2+=adcSingle1();
      adc_result2/=1000; //usrednjavanje radi boljeg odnosa signala/sum

      //Kada se prekrije foto => senzor pocinje merenje
      ...
      //Rezultat konverzije iz temperaturnog senzora
      ...
      if(msTicks>n*10000 && kalibrisan==1){
          U10= adc_result1;
        /*if(U10>Umax){
            Uoff2=U10;
        }*/

        //if(U10>=Umin1 || U10<(Umin1-10)){
          //if(U10>=Umin1){
              k=(U10-Umin1)/(10);
              y0=(-1)*k*(n-1)*10+Umin1;
              Utau=k*43.75+y0;
              voltage_predict=1.58*Utau+Uoff-450;
          /*}
          else{
              k=(Umax-U10)/(10);
              y0=k*(n-1)*10+Umax;
              Utau=k*43.75+y0;
              voltage_predict=1.58*Utau+Uoff2+450;
          }
        }
        else{
            voltage_predict=U10;
     ...
     //Pamti se najveca moguca ocekivana temperatura pred merenje
      if(! (GPIO_PinInGet(BUTTON_PORT, PB0))) {               // Ako je PB0 pritisnut
          GPIO_PinOutSet(LED_PORT, LED1);                        // pali se LED1
          Uoff=adc_result1;
          Umin1=Uoff;
          Umax=Uoff;
          //msTicks=0;
          ...
          GPIO_PinOutClear(LED_PORT, LED1);                      // gasi se LED1
      }
      ...
      OLEDI2C_clrScr();
      //Ispisivanje poruka i rezultata na displeju
      ...
      OLEDI2C_print(messageUmin, 50, 52);
      //OLEDI2C_print(messageUmax, 2, 52);
      ...
      OLEDI2C_printNumI(Uoff, 78 ,52 ,2,' ');
      //OLEDI2C_printNumI(Umax, 30 ,52 ,2,' ');
