...
/** "xTask2" priority */
#define mainxTask2_PRIO               ( 5 )
/** "ADC task" priority */
#define mainADC_TASK_PRIO             ( 5 )
/** "xTimerTask" priority */
#define mainxTaskTimer_PRIO           ( 5 )
/** "xTask1" priority */
#define mainxTask1_PRIO               ( 4 )
/** "xTask3" priority */
#define mainxTask3_PRIO               ( 3 )

#define PRIO_ISR_PORT1 3 /* the hardware priority level */
#define PRIO_ISR_PORT2 3 /* the hardware priority level */
#define PRIO_ISR_PORTADC 3 /* the hardware priority level */
...
traceHandle PORT1Handle;
traceHandle PORT2Handle;
...
{   int     previous_result=0;
    ...
    for ( ;; )
    {
        ...
        if(xQueueReceive(xTask3Mailbox, &newMessage, portMAX_DELAY) == pdTRUE){
...
void main( void )
{
    ...
    vTraceSetQueueName(xTask1Mailbox, "xTask1Mailbox");
    vTraceSetQueueName(xTask3Mailbox, "xTask3Mailbox");
    buttonCommand      =   BUTTON_S1;
    PORT1Handle = prvTraceGetObjectHandle(TRACE_CLASS_ISR);
    PORT1Handle = xTraceSetISRProperties("PORT1", PRIO_ISR_PORT1);
    PORT2Handle = prvTraceGetObjectHandle(TRACE_CLASS_ISR);
    PORT2Handle = xTraceSetISRProperties("PORT2", PRIO_ISR_PORT2);
    ADCHandle = prvTraceGetObjectHandle(TRACE_CLASS_ISR);
    ADCHandle = xTraceSetISRProperties("ADCPORT", PRIO_ISR_PORTADC);
    ue1 = xTraceRegisterString("S:");
    /* Start the scheduler. */
    vTaskStartScheduler();
...
void __attribute__ ( ( interrupt( PORT2_VECTOR  ) ) ) vPORT2ISR( void )
{
    /*Clear IFG register on exit. Read more about it in offical MSP430F5529 documentation*/
    P2IFG &=~0x02;
    vTraceStoreISRBegin(PORT2Handle);
    BaseType_t xHigherPriorityTaskWoken = pdFALSE;
